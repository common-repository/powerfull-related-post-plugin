...
function drpp_admin_menu() {
	$hook = add_options_page(__('Powerfull Related Posts PluginPlugin','drpp'),__('Powerfull Related Posts PluginPlugin','drpp'), 'manage_options', 'powerfull-related-post-plugin/options.php', 'drpp_options_page');
...
function drpp_settings_link($links, $file) {
  $this_plugin = dirname(plugin_basename(__FILE__)) . '/drpp.php';
  if($file == $this_plugin) {
...
class drpp_Widget extends WP_Widget {
  function drpp_Widget() {
    parent::WP_Widget(false, $name = __('Powerfull Related Posts PluginPlugin','drpp'));
  }
    ...
    echo $before_widget;
		if ( !$instance['use_template'] ) {
			...
			if ($title)
				...
				_e('Powerfull Related Posts PluginPlugin','drpp');
			echo $after_title;
    }
//    var_dump($instance);
  ...
  function form($instance) {				
    ...
    ?>
        ...
        <p><input class="checkbox" id="<?php echo $this->get_field_id('promote_drpp'); ?>" name="<?php echo $this->get_field_name('promote_drpp'); ?>" type="checkbox" <?php checked($instance['images'], true) ?> /> <label for="<?php echo $this->get_field_id('promote_drpp'); ?>"><?php _e("Help promote Powerfull Related Posts PluginPlugin?",'drpp'); ?></label></p>
