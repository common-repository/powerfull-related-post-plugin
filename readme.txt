=== Powerfull Related Posts PluginPlugin ===
Contributors: Rahul
Author: Rahul
...
Powerfull Related Posts PluginPlugin (drpp) gives you a list of posts and/or pages related to the current entry, introducing the reader to other relevant content on your site. Key features include:
...
1. Copy the folder `powerfull-related-post-plugin` into the directory `wp-content/plugins/` and (optionally) the sample templates inside `drpp-templates` folder into your active theme.
...
drpp is currently localized in the following languages:
  ...
  * Lithuanian (`lt_LT`) by [Karolis Vyčius](http://vycius.co.cc) (drpp-lt at mitcho dot com)
